...
def social_dist(xyxy, identities=[]):
    ...
    if len(identities)>=2:
        # Calculate every possible distance between top and bottom pairs of coordinates
        D_top = dist.cdist(top_cors, top_cors, metric="euclidean")
        D_bot = dist.cdist(bot_cors, bot_cors, metric="euclidean")

        for i in range(0, D_top.shape[0]):
            for j in range(i + 1, D_top.shape[1]):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    ...
                    draw_boxes(im0, bbox_xyxy, classes, identities)
                    # to MOT format
